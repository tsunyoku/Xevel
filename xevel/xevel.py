...
class Request:  # class to handle single request from client
    def __init__(self, client, loop):
        ...
        self.resp_headers: dict[
            Union[str, int],
            Any,
        ] = {}  # easy way to add headers to response :p
    ...
    async def _handle_headers(
        self,
        h: bytes,
    ) -> None:  # use _ for most internal functions ig xd
        headers = h.decode()
    ...
    def parse_form(self) -> None:
        ...
        for arg in b.split("&"):
            key, val = arg.split("=", 1)
            self.args[unquote(key).strip()] = unquote(
                val,
            ).strip()  # i am determined to find less ugly way of doing this
    ...
    def parse_multi(self) -> None:
        ...
        for part in p[:-1]:
            ...
            for key, val in [
                hd.split(": ", 1) for hd in [d for d in h.decode().split("\r\n")[1:]]
            ]:  # what the FUCK :smiley:
...
class Router:
    def __init__(self, domain: Union[str, set]):
        ...
        self.endpoints: set = (
            set()
        )  # endpoints current router handles (server can have multiple routers, useful for multi-file impl)
    ...
    def route(self, path: str, method: List[str] = ["GET"]):  # route decorator
        def wrapper(_coro: Coroutine):
            if all(
                c in path for c in ("<", ">")
            ):  # thank you lenforiee cus i absolutely hate regex
...
class Xevel:  # osu shall never leave my roots
    def __init__(self, address, **extras):
        ...
        self.loop: asyncio.AbstractEventLoop = extras.get(
            "loop",
            asyncio.get_event_loop(),
        )  # allow people to pass their own loop for whatever reason ig
    ...
    async def handle_route(self, req) -> None:
        ...
        if not router:
            return await req.send(
                code,
                resp,
            )  # couldn't find any router to handle request, return 404
        ...
        if isinstance(resp, (dict, list)):  # list usually contains dicts
            try:
                req.resp_headers[
                    "Content-Type"
                ] = "application/json"  # fix content type for browsers
        ...
        if (
            self.gzip
            and "Accept-Encoding" in req.headers
            and "gzip" in req.headers["Accept-Encoding"]
            and len(resp) > 1500
        ):
            if not (
                "Content-Type" in req.resp_headers
                and req.resp_headers["Content-Type"] in ("image/jpeg", "image/png")
            ):
        ...
        for _coro in router.after_reqs:
            await _coro(
                req,
            )  # handle any coroutines before ending request | send request so they can take some attributes from it
    ...
    def start(self) -> None:
        async def run_server() -> None:
            if isinstance(self.address, str):
                ...
                )  # raising exceptions kinda cooooooool

            if hasattr(socket, "AF_UNIX") and (
                t is socket.AF_UNIX and os.path.exists(self.address)
            ):
            ...
            if hasattr(socket, "AF_UNIX") and t is socket.AF_UNIX:
                os.chmod(
                    self.address,
                    0o777,
                )  # full permissions to socket file to prevent any potential perm issues xd

            self.socket.listen()

            while True:
                request, _ = await self.loop.sock_accept(self.socket)
                self.loop.create_task(self.handle_req(request))

        try:
            self.loop.run_until_complete(run_server())
        except KeyboardInterrupt:
            self.loop.run_until_complete(self.cleanup_loop())
